defmodule Finder do
  ...
  def find_shortest(dist_map, location_list) do
    {reversed_path, dist} = find_shortest(dist_map, location_list, [], 0)
    {reversed_path, dist}
  end
  def find_shortest(_dist_map, [], path, path_dist), do: {path, path_dist}
  def find_shortest(dist_map, location_list, path, path_dist) do
    location_list
    |> Enum.map(fn location ->
      ...
      find_shortest(dist_map, new_location_list, [location | path], dist + path_dist)
    end)
  ...
  def calc_result(input) do
    ...
    {_path, result} = Finder.find_shortest(dist_map, location_list)
    # |> IO.inspect
